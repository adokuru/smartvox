...
if (isset($_POST['DTHamount'])) 
	{
		$apibase = 'https://payments.baxipay.com.ng/api/baxipay/services/databundle/request';
		$phonenumber = $_POST['dataNumber'];
		$amount = $_POST['DTHamount'];
		$code = $_POST['datacode'];
		$network = $_POST['dataoperator'];
		$agentReference = 'Smartvox'.substr(str_shuffle(md5(time())), 0, 20);
		$url  = $apibase.'?phone='.$phonenumber.'&amount='.$amount.'&service_type='.$network.'&datacode='.$code.'&agentId=212&agentReference='.$agentReference;
		$curl = curl_init();
